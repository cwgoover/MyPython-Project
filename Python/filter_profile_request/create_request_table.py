...
TITLE = (UTDID, OFFLINE, MAT, VIDENULL, APP_VERION)
...
def prepare_dic(file_str):
    ...
    for idx, val in enumerate(item_list):  # 返回list的index
        ...
        if idx == 0 or idx == 1 or idx == 2 or idx == 3 or idx == 5:
...
def split_dict_by_offline(raw_dic):
    ...
    write_dict_to_file(offline_dic, online_dic, invalid_dic)
    return offline_dic, online_dic


def write_dict_to_file(offline_dic, online_dic, invalid_dic):
    offline_file = "offline-dic.txt"
    silent_remove(offline_file)
    write_content(offline_file, str('\n *** OFFLINE ****:\n'))
    write_content(offline_file, str("** size: {}\n".format(len(offline_dic.get(UTDID)))))
    write_to_file(offline_dic, offline_file)

    online_file = "online-dic.txt"
    silent_remove(online_file)
    write_content(online_file, str('\n *** ONLINE ****:\n'))
    write_content(online_file, str("** size: {}\n".format(len(online_dic.get(UTDID)))))
    write_to_file(online_dic, online_file)

    if any(invalid_dic):
        invalid_file = "invalid-dic.txt"
        silent_remove(invalid_file)
        write_content(invalid_file, str('\n *** INVALID ****:\n'))
        write_to_file(invalid_dic, invalid_file)
